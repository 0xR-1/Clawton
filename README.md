...
### Setup steps
1. Clone the repo and copy each `.env.example` to `.env` in `newton-policy-wallet/` and `.env.policy.example` to `.env.policy` in `policy-workspace/`, filling in your own wallet key and RPC URLs.
2. Install Foundry dependencies: `cd newton-policy-wallet && forge install`
3. Install Node dependencies: `cd guard && npm install` and `cd x402-demo && npm install`
4. Deploy your own policy and wallet contracts by following the deployment commands referenced in the Architecture section above — this project intentionally does not include a shared, pre-deployed instance for other users to call into.
5. Run `node guard/clawton-guard.js '{"symbol":"BTCUSDT","side":"BUY","quantity":"0.001"}'` to test the Binance path, or start `x402-demo/server.js` and run `node x402-demo/clawton-guard-x402.js <resource-url>` to test the x402 path.
